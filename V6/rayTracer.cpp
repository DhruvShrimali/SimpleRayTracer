...
colour recursive(vect& orig, vect& dir, int bounces, std::vector<Shape*> *objects){
    ...
    float probability_of_refraction = object->transparency;
    float random_probability = randomProbability();
    ...
    if (object->transparency > 0.01) {
        ...
        if(probability_of_refraction >= random_probability){
            vect refractDir;
            vect refractOrig;
            if (enteringMedium) // when ray entering sphere
                refractOrig = intersection - (normal * 0.0001); // Start slightly inside the sphere
            ...
            refractionColor.mult(object->energy_retention);
...
int main(int argc, char* argv[]) {
    ...
    time = (int)time % 60;
    int time_hour = (int) (time_min / 60);
    time_min = (int)time_min % 60; 

    
    printf("Please wait for less than %d hr, %d min, %.0lf sec\n",time_hour, time_min, time);
    ...
    do{
        ...
        if(!checked){
            ...
            expected_epochs = expected_epochs<EPOCHLIMIT? expected_epochs:EPOCHLIMIT;
        }
        std::cout<<"Epoch "<<EPOCHS<<" / "<<expected_epochs<<" completed\n";
    } 
    // while(epoch_duration.count() < TIMELIMIT);
    while((epoch_duration.count() < ((int)time_remaining-(int)time)) && (EPOCHS < EPOCHLIMIT));
